...
**Determinism (pass 2).** `best_col`'s "the max's query column" **must** reuse the existing `index_of` flat-scan convention (lowest column achieving the max, `fill.rs:87-102`), *not* a within-vector argmax. A lane-order argmax is `LANES`-dependent and would differ between AVX2 (16 lanes) and SSE4.1 (8 lanes). The `R` reverse-pass tie-break is already pinned (lowest rank); min/max unions are order-free. With those two rules the band *geometry inputs* — `R`, `anchor`, `best_col`, and the half-open column window `[beg, end)` — are `LANES`-independent (identical on every ISA).

**Caveat — the computed region is still `LANES`-quantized.** The window is realized in the striped fill at segment granularity: `[beg_sn, end_sn) = [beg / LANES, ⌈end / LANES⌉)`, so the *cells actually computed* span `[beg_sn·LANES, end_sn·LANES)`, which **is** `LANES`-dependent (a narrow window rounds out to at least one full segment — up to 16 columns on AVX2 vs 8 on SSE4.1/NEON). Consequently a banded alignment's *result* (score/path) may differ across ISAs when the true optimum sits just outside the column window but inside a wider ISA's segment-rounding. This is acceptable for a heuristic — every ISA still satisfies `banded ≤ exact` (the safety invariant), and the exact/default path is unaffected — but it means banded results are **not** guaranteed byte-identical across ISAs, and cross-ISA tests must assert only `LANES`-robust properties (e.g. `banded ≤ exact`, or a "must miss" premise sized to exceed even the widest segment-rounded band), never an exact banded score/path that assumes a particular lane width.
